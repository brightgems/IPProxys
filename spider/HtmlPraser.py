...
class Html_Parser(object):
    ...
    def XpathPraser(self,response,parser):
        ...
        for proxy in proxys:
            ...
            # ip，端口，类型(0高匿名，1透明)，protocol(0 http,1 https
            # http),country(国家),area(省市),updatetime(更新时间)

            # proxy
            # ={'ip':ip,'port':int(port),'type':int(type),'protocol':int(protocol),'country':country,'area':area,'updatetime':updatetime,'speed':100}
            proxy = {'source':source,'ip':ip,'port':int(port),'type':int(type),'protocol':int(protocol),'country':country,'area':area,'updatetime':updatetime,'speed':100}
    ...
    def RegularPraser(self,response,parser):
        ...
        if matchs != None:
            for match in matchs:
                ...
                updatetime = datetime.datetime.now()
                proxy = {'source':source ,'ip':ip,'port':port,'type':type,'protocol':protocol,'country':country,'area':area,'updatetime':updatetime,'speed':100}
                logger.info("Fetch proxy %s" % str(proxy))
                proxylist.append(proxy)
            return proxylist
